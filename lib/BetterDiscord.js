...
async function load(reload) {
    ...
    if(!reload) {
        if(_cfg.updater.LatestVersion > _cfg.version) {
            utils.alert("Update Available", "An update for BandagedBD is available ("+_cfg.updater.LatestVersion+")! <a href='https://github.com/rauenzi/BetterDiscordApp/releases' target='_blank'>Download Installer</a>");
        }
    ...
    try {
		...
		await utils.execJs(`
			(async() => {
				...
				while (!window.webpackJsonp.flatten || !window.webpackJsonp.flatten().flatten) await new Promise(r => setTimeout(r, 100));
				while (window.webpackJsonp.flatten().flatten().length < 4000) await new Promise(r => setTimeout(r, 100));
			})();
		`);
