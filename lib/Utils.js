...
Utils.prototype.checkConnection = function(url) {
    return new Promise(function(resolve, reject) {
        var options = {
            hostname: url, port: 443,
            path: '/', method: 'HEAD'
        };
...
Utils.prototype.getCDN = function(current) {
    return new Promise(resolve => {
        this.checkConnection(current).then(
            _ => { //Succ
                resolve(current);
            },
            _ => { //Fail
                resolve('rauenzi.github.io');
            }
        );
    });
}
...
Utils.prototype.getUpdater = function(repo, hash, callback) {
    var getCDN = this.getCDN.bind(this);
...
Utils.prototype.execJs = function(js) {
    return this.getWebContents().executeJavaScript(js);
}

//Parse and execute javascript
Utils.prototype.execJsParse = function(js) {
    return this.execJs(js); //TODO
}
...
Utils.prototype.alert = function(title, message) {
	...
	var bdAlert = `<div class="bd-modal" id="${id}">
				<div class="backdrop-1wrmKB" style="opacity: 0.85; background-color: rgb(0, 0, 0); z-index: 1000; transform: translateZ(0px);" onclick="${closeAction}"></div>
						...
						<div class="modal-3HD5ck modal-1sor29 size-JLvFeT">
							...
							<div class="scrollerWrap-2lJEkd content-2BXhLs scrollerThemed-2oenus themeGhostHairline-DBD-2d">
								<div class="scroller-2FKFPG inner-3wn6Q5 content-8biNdB selectable">
									<div class="medium-zmzTW- size16-14cGz5 height20-mO2eIN primary-jw0I4K selectable-x8iAUj">
${message}
									</div>
								</div>
							</div>
    ...
    this.execJs(`document.querySelector("#app-mount>.theme-dark:last-of-type") && document.querySelector("#app-mount>.theme-dark:last-of-type").insertAdjacentHTML('afterbegin', \`${bdAlert}\`);`);
}
