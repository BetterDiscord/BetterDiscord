...
Utils.prototype.attemptSync = function(func, attempts, attempt, message, success, err) {
    ...
    setTimeout(function() {
        if(!func()) {
            ...
            self.attemptSync(func, attempts, attempt, message, success, err);
            return;
        }
...
Utils.prototype.attempt = function(func, attempts, attempt, message, success, err) {
    ...
    setTimeout(function() {
        func(function(ok, errMsg) {
            if(!ok) {
                self.warn(message + " [" + errMsg + "], retrying #" + attempt);
                self.attempt(func, attempts, attempt, message, success, err);
                return;
            }
            success();
        });
