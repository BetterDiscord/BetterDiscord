...
const releaseInput = useBdRelease
    ? args[3] && args[3].toLowerCase()
    : args[2] && args[2].toLowerCase();
const release = releaseInput === "canary"
    ? "Discord Canary"
    : releaseInput === "ptb"
        ? "Discord PTB"
        : "Discord";
const bdPath = useBdRelease
    ? path.resolve(__dirname, "..", "dist", "betterdiscord.asar")
...
const discordPath = (() => {
    ...
    if (process.platform === "win32") {
        ...
        const coreWrap = fs
            .readdirSync(path.join(basedir, version, "modules"))
        ...
        let userData = process.env.XDG_CONFIG_HOME
