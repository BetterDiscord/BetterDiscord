...
export default class Patcher {
    ...
    static makeOverride(patch) {        
        return new Proxy(patch.originalFunction, {
            apply(target, thisArg, argArray) {
                let returnValue;
                if (!patch.children || !patch.children.length) return patch.originalFunction.apply(thisArg, argArray);
                for (const superPatch of patch.children.filter(c => c.type === "before")) {
                    try {
                        superPatch.callback(thisArg, argArray);
                    }
                    catch (err) {
                ...
                if (!insteads.length) {returnValue = patch.originalFunction.apply(thisArg, argArray);}
                else {
                    for (const insteadPatch of insteads) {
                        try {
                            const tempReturn = insteadPatch.callback(thisArg, argArray, patch.originalFunction.bind(thisArg));
                ...
                for (const slavePatch of patch.children.filter(c => c.type === "after")) {
                    try {
                        const tempReturn = slavePatch.callback(thisArg, argArray, returnValue);
                        if (typeof(tempReturn) !== "undefined") returnValue = tempReturn;
                    }
                    catch (err) {
    ...
    static makePatch(module, functionName, name) {
        ...
        patch.proxyFunction.displayName = `BDPatcher(${patch.proxyFunction.displayName || patch.proxyFunction.name || ""})`;
        // If plugins want to access the unpatched?
