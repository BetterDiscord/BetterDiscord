...
const ErrorDetails = ({componentStack, pluginInfo, stack, instance}) => {
    ...
    const openGithubIssue = () => {
        if (pluginInfo?.githubUrl) {
            const issueTitle = encodeURIComponent(`[Bug Report] Plugin Crash - ${pluginInfo.name}`);
            const issueBody = encodeURIComponent(
                `### Error Details\n\`\`\`\n${stack}\n\`\`\`\n - Generated by BD Recovery Builtin.` + "\n\n### Steps to Reproduce\n1.\n2.\n3.\n\n" + "### Additional Context\n"
            );
            window.open(`${pluginInfo.githubUrl}/issues/new?title=${issueTitle}&body=${issueBody}`, "_blank");
        }
    };
    ...
    getPluginInfo(pluginName) {
        try {
            ...
            return {
                ...
                githubUrl: plugin.github || plugin.source,
