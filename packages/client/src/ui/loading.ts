import DOMManager from "../modules/dommanager";

const css = `
#bd-loading-icon {
    background-image: url(data:image/svg+xml;base64,);
}

#bd-loading-icon {
    position: fixed;
    bottom:5px;
    right:5px;
    z-index: 2147483647;
    display: block;
    width: 20px;
    height: 20px;
    background-size: 100% 100%;
    animation: bd-loading-animation 1.5s ease-in-out infinite;
}

@keyframes bd-loading-animation {
    0% { opacity: 0.05; }
    50% { opacity: 0.6; }
    100% { opacity: 0.05; }
}`;

export default class LoadingIcon {
    private static element: null | HTMLElement = null;

    public static show() {
        DOMManager.appendStyle("loading-icon", css);

        this.element = DOMManager.createElement("div", {
            id: "bd-loading-icon",
            title: "BetterDiscord is loading..."
        });
        
        DOMManager.documentReady.then(() => {
            document.body.appendChild(this.element as Node);
        });
    }

    public static hide() {
        DOMManager.removeStyle("loading-icon");
        this.element?.remove();
    }
}
