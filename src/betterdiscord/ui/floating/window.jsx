...
export default function FloatingWindow({
    ...
    const [position, setPosition] = useState({
        x: center ? Screen.width / 2 - initialWidth / 2 : initialLeft,
        y: center ? Screen.height / 2 - initialHeight / 2 : initialTop,
    });
    ...
    const onResizeStart = useCallback(() => {
        setSize({
            width: window.current.offsetWidth,
            height: window.current.offsetHeight,
        });
    }, [window]);
    ...
    const onDragStart = useCallback(
        (e) => {
            ...
            setOffset({
                x: e.clientX - Number.parseInt(div.offsetLeft),
                y: e.clientY - Number.parseInt(div.offsetTop),
            });
            setDragging(true);
        },
        [window],
    ...
    const onDragStop = useCallback(() => {
        ...
        if (width != size.width || height != size.height) {
            ...
            const left = Number.parseInt(window.current.style.left);
            const top = Number.parseInt(window.current.style.top);
    ...
    const maximize = useCallback(() => {
        ...
        const newLeft = Number.parseInt(window.current.style.left);
        const newTop = Number.parseInt(window.current.style.top);
    ...
    return (
        <div id={id} className={finalClassname} ref={window} style={styles}>
