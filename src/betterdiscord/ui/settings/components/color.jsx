...
const getRGB = (color) => {
    let result = /rgb\(\s*([0-9]{1,3})\s*,\s*([0-9]{1,3})\s*,\s*([0-9]{1,3})\s*\)/.exec(
        color,
    );
    if (result) {
        return [
    ...
    if (result) {
        return [
            Number.parseFloat(result[1]) * 2.55,
            Number.parseFloat(result[2]) * 2.55,
            Number.parseFloat(result[3]) * 2.55,
        ];
    }
    ...
    if (result) {
        return [
            Number.parseInt(result[1] + result[1], 16),
            Number.parseInt(result[2] + result[2], 16),
            Number.parseInt(result[3] + result[3], 16),
        ];
    }
};
...
export default function Color({
    ...
    return (
        <div
            className={`bd-color-picker-container ${disabled ? "bd-color-picker-disabled" : ""}`}
        >
                    ...
                    <DiscordModules.Tooltip text="Default" position="bottom">
                        {(props) => (
                            <div
                                ...
                                style={{backgroundColor: resolveColor(defaultValue)}}
                                onClick={() => change({target: {value: defaultValue}})}
                            >
                                {intValue === resolveColor(defaultValue, false) ? (
                                    <CheckIcon
                                        size="25px"
                                        color={getContrastColor(resolveColor(defaultValue, true))}
                                    />
                                ) : null}
                            </div>
                        )}
                ...
                >
                    {(props) => (
                        <div className="bd-color-picker-custom">
                            <PipetteIcon
                                size="14px"
                                color={getContrastColor(resolveColor(value, true))}
                            />
                            <input
                                {...props}
                ...
                <div className="bd-color-picker-swatch">
                    {colors.map((int, index) => (
                        <div
                            ...
                            onClick={() => change({target: {value: int}})}
                        >
                            {intValue === int ? (
                                <CheckIcon
                                    size="16px"
