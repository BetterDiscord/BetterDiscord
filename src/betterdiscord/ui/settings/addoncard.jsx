...
function makeButton(
    ...
    children,
    action,
    {isControl = false, danger = false, disabled = false} = {},
) {
    const ButtonType = isControl ? "button" : "div";
    return (
        <DiscordModules.Tooltip color="primary" position="top" text={title}>
            {(props) => {
                return (
                    <ButtonType
                        {...props}
                        ...
                        className={
                            (isControl
                                ? "bd-button bd-button-filled bd-addon-button"
                                : "bd-addon-button") + (danger
                                    ? " bd-button-color-red"
                                    : isControl
                                        ? " bd-button-color-brand"
                                        : "") + (disabled ? " bd-button-disabled" : "")
                        }
                        onClick={action}
                        disabled={disabled}
                    >
...
export default function AddonCard({
    ...
    const showSettings = useCallback(() => {
        ...
        try {
            ...
            Logger.stacktrace(
                "Addon Settings",
                "Unable to get settings panel for " + name + ".",
                err,
            );
        }
    ...
    const messageAuthor = useCallback(() => {
        ...
        if (
            !UserStore || !ChannelActions || !ChannelStore || !PrivateChannelActions
        ) {return;}
        ...
        return (
            <div className="bd-footer">
                ...
                <div className="bd-controls">
                    {hasSettings
                        && makeButton(
                            t("Addons.addonSettings"),
                            <SettingsIcon size={"20px"} />,
                            showSettings,
                            {isControl: true, disabled: !enabled},
                        )}
                    {editAddon
                        && makeButton(
                            t("Addons.editAddon"),
    ...
    return (
        <div
            id={`${addon.id}-card`}
            className={"bd-addon-card" + (disabled ? " bd-addon-card-disabled" : "")}
        >
            <div className="bd-addon-header">
                {type === "plugin" ? (
                    <PlugIcon size="20px" className="bd-icon" />
                ) : (
                    <PaletteIcon size="20px" className="bd-icon" />
                )}
            ...
            <div className="bd-description-wrap">
                {disabled && (
                    <div className="banner banner-danger">
                        <ShieldAlertIcon className="bd-icon" />
