...
const ADDON_REGEX = new RegExp(
    [
        ...
        /https?:\/\/betterdiscord\.app\/(?:theme|plugin)(?:\/([^/\s]+)\/?|\?id=(\d+))/
            .source,
    ].join("|"),
    "gi",
);
 ...
 */
function extractAddonLinks(text, max = Number.POSITIVE_INFINITY) {
...
export default new (class AddonStoreBuiltin extends Builtin {
    constructor() {
        ...
        Settings.on(this.collection, this.category, "addonEmbeds", () =>
    ...
    initialize() {
        RemoteAPI.setProtocolListener((url) => {
            ...
            AddonStore.requestAddon(decodeURIComponent(match[1])).then((addon) =>
                addon.download(),
            );
        });
    ...
    forceUpdateChat() {
        for (const message of document.querySelectorAll("[id^=chat-messages-]")) {
            ...
            const child = findInTree(
                instance,
                ($child) => typeof $child?.memoizedProps?.onMouseLeave === "function",
                {
                    walkable: ["child"],
                },
            );
            ...
            for (const forward of message.querySelectorAll(
                "[id^=\"message-accessories-\"] [id^=\"message-accessories-\"]",
            )) {
    ...
    patchMarkdown() {
                ...
                return React.createElement(
                    "a",
                    {
                        ...
                        onClick(event) {
                            ...
                            AddonStore.requestAddon(node.exec[1]).then((addon) =>
                                addon.download(false),
                            );
                        },
                    },
        ...
        SimpleMarkdownWrapper.parse = SimpleMarkdownWrapper.reactParserFor(
            SimpleMarkdownWrapper.defaultRules,
        );
    }

    async patchEmbeds() {
        MessageAccessories ??= await getLazy(
            Filters.byPrototypeKeys(["renderEmbeds"]),
            {searchExports: true},
        );
        ...
        this.after(
            ...
            (_, [message], res) => {
                ...
                ) {type = Web.getReleaseChannelType(message.messageReference.channel_id);}

                if (type) {
                    const id = message.embeds[0].rawDescription
                        ?.split?.("\n")
                        ?.at?.(-1)
                        ?.match?.(/\?id=(\d+)/);
                    ...
                    if (id) {
                        return React.createElement(
                            ErrorBoundary,
                            null,
                            React.createElement(AddonEmbed, {id: id[1], original: res}),
                        );
                    }
                ...
                if (matches.length) {
                    ...
                    for (let key = 0; key < matches.length; key++) {
                        ...
                        for (let embedIndex = 0; embedIndex < res.length; embedIndex++) {
                            ...
                            if (embed?.url === match) {
                                shouldAdd = false;
                                embeds[embedIndex] = React.createElement(
                                    ErrorBoundary,
                                    {key},
                                    React.createElement(AddonEmbed, {
                                        id,
                                        original: embeds[embedIndex],
                                    }),
                                );
                                break;
                            }
                        ...
                        if (shouldAdd) {
                            embeds.push(
                                React.createElement(
                                    ErrorBoundary,
                                    {key},
                                    React.createElement(AddonEmbed, {id}),
                                ),
                            );
                        }
                    }
