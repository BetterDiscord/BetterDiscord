...
import type {
    ...
    Options,
    WithKeyOptions,
} from "discord/webpack";

type WithOptions<T, B extends WebpackOptions> = [...T[], B] | T[];

const getOptions = <T, B extends Options>(
    args: WithOptions<T, B>,
    defaultOptions: B = {} as B,
): [T[], B] => {
    if (
        args.length > 1
...
const Webpack = {
    ...
    getWithKey(filter: ExportedOnlyFilter, options: WithKeyOptions = {}) {
        if ("first" in options) {
            return Logger.error(
                ...
                "Unsupported option first.",
            );
        }
        if (
            "defaultExport" in options
            && typeof options.defaultExport !== "boolean"
        ) {
            return Logger.error(
                "BdApi.Webpack~getWithKey",
        ...
        if (
            "searchExports" in options
            && typeof options.searchExports !== "boolean"
        ) {
            return Logger.error(
                "BdApi.Webpack~getWithKey",
        ...
        if ("raw" in options && typeof options.raw !== "boolean") {
            return Logger.error(
                "BdApi.Webpack~getWithKey",
                "Invalid type for options.raw",
                options.raw,
                "Expected: boolean",
            );
        }
    ...
    getModule<T>(filter: Filter, options: WebpackOptions = {}) {
        if ("first" in options && typeof options.first !== "boolean") {
            return Logger.error(
                "BdApi.Webpack~get",
                "Invalid type for options.first",
                options.first,
                "Expected: boolean",
            );
        }
    ...
    getBulk<T extends any[]>(...queries: BulkQueries[]) {
        return getBulk<T>(...queries);
    },
    ...
    waitForModule<T>(filter: Filter, options: LazyOptions = {}) {
        ...
        if ("signal" in options && !(options.signal instanceof AbortSignal)) {
            return Logger.error(
                "BdApi.Webpack~waitForModule",
                "Invalid type for options.signal",
                options.signal,
                "AbortSignal expected.",
            );
        }
        if (
            "searchExports" in options
    ...
    getAllByRegex<T extends any[]>(regex: RegExp, options: WebpackOptions = {}) {
        return Webpack.getModule<T>(
            Filters.byRegex(regex),
            Object.assign({}, options, {first: false}),
        );
    },
    ...
    getAllByPrototypeKeys<T extends any[]>(
        ...prototypes: WithOptions<string, WebpackOptions>
    ) {
        const [keys, options] = getOptions(prototypes);

        return Webpack.getModule<T>(
            Filters.byPrototypeKeys(keys),
            Object.assign({}, options, {first: false}),
        );
    },
    ...
    getAllByStrings<T extends any[]>(
        ...strings: WithOptions<string, WebpackOptions>
    ) {
        const [keys, options] = getOptions(strings);

        return Webpack.getModule<T>(
            Filters.byStrings(...keys),
            Object.assign({}, options, {first: false}),
        );
    },
    ...
    getAllBySource<T extends object[]>(
        ...searches: WithOptions<string | RegExp, WebpackOptions>
    ) {
        const [keys, options] = getOptions(searches);

        return Webpack.getModule<T>(
            Filters.bySource(...keys),
            Object.assign({}, options, {first: false}),
        );
    },
