...
const getOptions = <T, B extends Webpack.Options>(args: WithOptions<T, B>, defaultOptions: B = {} as B): [ T[], B ] => {
    if (args.length > 1
...
const Webpack = {
    ...
    Filters: {
        ...
        combine(...filters: Webpack.Filter[]): Webpack.Filter  {return Filters.combine(...filters);},
    },

    getWithKey(filter: Webpack.ExportedOnlyFilter, options: Webpack.WithKeyOptions = {}) {
    ...
    getModule<T extends any>(filter: Webpack.Filter, options: WebpackOptions = {}) {
    ...
    getModules<T extends any[]>(filter: Webpack.Filter, options: WebpackOptions = {}) {
    ...
    getBulk<T extends any[]>(...queries: Webpack.BulkQueries[]) {return getBulk<T>(...queries);},

    waitForModule<T>(filter: Webpack.Filter, options: Webpack.LazyOptions = {}) {
