...
const ContextMenuActions = (() => {
    ...
    try {
        const ActionsModule = getModule(
            (mod, target, id) =>
                modules[id]?.toString().includes(`type:"CONTEXT_MENU_OPEN"`),
            {searchExports: false},
        );
        ...
        startupComplete &&= typeof out.closeContextMenu === "function"
            && typeof out.openContextMenu === "function";
    }
    catch (error) {
...
class MenuPatcher {
    ...
    static initialize() {
        if (!startupComplete) {
            return Logger.warn(
                "ContextMenu~Patcher",
                "Startup wasn't successfully, aborting initialization.",
            );
        }
            ...
            const foundModule = getModule(
                (m) =>
                    Object.values(m).some(
                        (v) =>
                            typeof v === "function"
                            && v.toString().includes(`type:"CONTEXT_MENU_CLOSE"`),
                    ),
                {searchExports: false},
            );
            const foundKey = Object.keys(foundModule).find(
                (k) => foundModule[k].length === 3,
            );
        ...
        const proxyFunction = this.subPatches.get(target[method]) ?? (() => {
            ...
            function patch() {
                ...
                if (res.props?.navId ?? res.props?.children?.props?.navId) {
                    MenuPatcher.runPatches(
                        res.props.navId ?? res.props?.children?.props?.navId,
                        res,
                        arguments[0],
                    );
                }
    ...
    static runPatches(id, res, props) {
        ...
        for (const patch of this.patches[id]) {
            try {
                ...
                Logger.error(
                    "ContextMenu~runPatches",
                    `Could not run ${id} patch for`,
                    patch,
                    error,
                );
            }
    ...
    buildItem(props) {
        ...
        if (type === "submenu") {
            ...
            Component = type === "toggle"
                ? MenuComponents.CheckboxItem
                : MenuComponents.RadioItem;
