...
class V2C_PluginCard extends BDV2.reactComponent{constructor(props){super(props);let self=this;self.onChange=self.onChange.bind(self);self.showSettings=self.showSettings.bind(self);self.setInitialState();}
setInitialState(){this.state={'checked':pluginCookie[this.props.plugin.getName()],'settings':false};}
componentDidUpdate(){if(this.state.settings){}}
render(){let{plugin}=this.props;let name=plugin.getName();let author=plugin.getAuthor();let description=plugin.getDescription();let version=plugin.getVersion();if(this.state.settings){return BDV2.react.createElement("li",{style:{maxHeight:"500px",overflow:"auto"}},BDV2.react.createElement("div",{style:{float:"right",cursor:"pointer"},onClick:()=>{this.setState({'settings':false});}},BDV2.react.createElement(V2Components.XSvg,null)),BDV2.react.createElement("div",{ref:"settingspanel",dangerouslySetInnerHTML:{__html:plugin.getSettingsPanel()}}));}
...
showSettings(){this.setState({'settings':true});}}
...
return _root[0];}
injectRoot(){let changeLog=$(".ui-tab-bar-item:contains('Change Log')");if(!changeLog.length)return false;$("<span/>",{'id':'bd-settings-sidebar'}).insertBefore(changeLog.prev());return true;}
